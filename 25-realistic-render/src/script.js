...
gltfLoader.load(
    ...
    (gltf) => {
        ...
        gui.add(gltf.scene.rotation, 'y').min(- Math.PI).max(Math.PI).step(0.001).name('roatation')
        updateAllMaterials()
    }
)
...
directionalLight.shadow.normalBias = 0.7
...
renderer.setSize(sizes.width, sizes.height)
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2))
renderer.physicallyCorrectLights = true // real-life condition, 블렌더와 맞추기
